...
impl Note {
    ...
    ///
    /// **32 字节词 → KoalaBear 域元素的规范映射**：KoalaBear 素数为
    /// 31-bit（p = 2^31 − 2^24 + 1，见 p3-koala-bear 0.7.0-rc.1），单个
    /// 32 字节词不是单域元素。Task 10（NoteHasher 实现）与 Task 11
    /// （电路）中，每个 32 字节词按确定性映射拆分为 **8 个连续小端 32-bit
    /// limb**，即词 b[0..32] → u32_le(b[0..4]) .. u32_le(b[28..32])，每个
    /// limb 经域的规范归约（from_u64 语义）载入；6 词共 48 个域元素。
